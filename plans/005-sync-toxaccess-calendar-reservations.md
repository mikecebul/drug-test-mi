...
> **Architecture update — July 29, 2026:** Anonymous placeholders use the
> Google Calendar API directly and have no attendees or reminders. Cal.com
> remains the source of recurring availability and date overrides. After
> ToxAccess identifies the donor, the daily task creates the real unpaid
> `midrugtest/drug-test` Cal.com booking (event type `3684719`), writes Today’s
> Schedule, and then deletes the placeholder. The implementation and deployment
> instructions in `docs/redwood-automation-improvements.md` supersede the older
> Cal.com-hold design exploration retained below.
...
- Monday's scheduled task creates metadata-tagged anonymous Google Calendar
  holds from the two-week aggregate page.
- The daily task creates an unpaid booking through API v2 using the existing
  `midrugtest/drug-test` event type `3684719`, links it by exact donor ID,
  writes the Payload booking used by Today's Schedule, and only then deletes
  the corresponding tagged Google Calendar hold.
...
- Anonymous holds never pass through Cal.com, so they cannot create webhook
  rows or attendee notifications.
