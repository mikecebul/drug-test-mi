...
import { getRandomTestingStart } from '@/lib/random-testing/calcom'
...
async function checkCalcom(collectionDate: string): Promise<ServiceCheck> {
  try {
    const timing = await getRandomTestingStart({ collectionDate, slotIndex: 0 })
    return {
      name: 'Cal.com',
      success: true,
      detail: `First slot for ${collectionDate}: ${new Date(timing.start).toLocaleString('en-US', {
