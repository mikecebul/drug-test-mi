...
export const ConfirmFieldGroup = withFieldGroup({
  ...
  render: function Render({ group, title, description = '' }) {
    ...
    const uploadTestType = uploadData?.wizardType as WizardType
    ...
    const { data: client } = useGetClientFromTestQuery(verifyTest?.testId)
                ...
                extractData.confirmationResults.length > 0 && (
                  <div className="space-y-2 border-t pt-2">
                    ...
                    <div className="space-y-2 pl-6">
                      {extractData.confirmationResults.map((result: any, index: number) => {
                        const resultConfig = {
                          'confirmed-positive': {
                        ...
                        <Alert className="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/30">
                          ...
                          <AlertDescription>
                            ...
                            <p className="mt-1 text-sm text-green-800 dark:text-green-200">
                              No substances detected and no medications expected. This result will
              ...
              {verifyData?.breathalyzerTaken && verifyData?.breathalyzerResult !== null && (
                <div className="space-y-2 border-t pt-2">
                  ...
                  <div className="pl-6">
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={verifyData.breathalyzerResult > 0.0 ? 'destructive' : 'default'}
                        className={
                          verifyData.breathalyzerResult > 0.0 ? 'gap-1' : 'gap-1 bg-green-600'
                        }
                      >
                        {verifyData.breathalyzerResult > 0.0 ? (
                          <>
                            <XCircle className="h-3 w-3" />
                            POSITIVE (FAIL)
                          </>
                    ...
                    <p className="mt-2 text-sm">
                      ...
                      <span className="font-mono font-semibold">
                        {verifyData.breathalyzerResult.toFixed(3)}
                      </span>
                    </p>
                    {verifyData.breathalyzerResult > 0.0 && (
