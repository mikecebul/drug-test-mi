...
export function ClientInfoCard({
  ...
  const fullName = [client.firstName, client.middleInitial ? `${client.middleInitial}. ` : '', client.lastName]
    .filter(Boolean)
    .join(' ')
  ...
  const content = (
    <div className={cn('flex items-start', gapClass)}>
  ...
  return (
    <div className={cn('bg-card border-border w-full rounded-xl border p-6 shadow-md', className)}>
      {content}
    </div>
  )
}
