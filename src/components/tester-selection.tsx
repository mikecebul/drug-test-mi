...
import { getAvailableTesters, getTesterAvailabilityText } from "@/lib/mock-data"
import type { GenderPreference, TimePreference, DayPreference, TesterWithAvailability } from "@/lib/types"

interface TesterSelectionProps {
  onTesterSelect: (tester: TesterWithAvailability) => void
}

export function TesterSelection({ onTesterSelect }: TesterSelectionProps) {
  ...
  const availableTesters = getAvailableTesters(
    [genderPreference],
    [timePreference],
    [dayPreference],
    locationPreference,
        ...
        <div className="space-y-1.5">
          ...
          <Select value={locationPreference} onValueChange={setLocationPreference}>
            ...
            <SelectContent>
              ...
              <SelectItem value="charlevoix">Charlevoix</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </CardContent>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            ...
            <Button
              variant={locationPreference === "charlevoix" ? "default" : "outline"}
              size="sm"
              onClick={() => setLocationPreference("charlevoix")}
              className="w-full justify-start h-8 text-sm font-medium"
            >
              <MapPin className="h-3 w-3 mr-2" />
              Charlevoix
        ...
        <div className="pt-3 border-t">
          <div className="text-xs text-muted-foreground">
            <span className="font-semibold text-foreground text-sm">{availableTesters.length}</span> tester
            {availableTesters.length !== 1 ? "s" : ""} available
          </div>
        </div>
      </CardContent>
    </Card>
      ...
      <div className="md:grid md:grid-cols-4 md:gap-6 lg:gap-8">
        ...
        <div className="md:col-span-3 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold md:hidden">Available Testers ({availableTesters.length})</h3>
            <h3 className="text-xl font-semibold hidden md:block">Available Testers</h3>
          </div>
          ...
          {availableTesters.length === 0 ? (
            <Card>
              <CardContent className="pt-8 pb-8 text-center">
                <p className="text-muted-foreground">
                  No testers available for your selected preferences. Please try different filters.
                </p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-3 lg:gap-4">
              {availableTesters.map((tester) => (
                <Card key={tester.id} className="hover:shadow-lg transition-all duration-200 hover:border-primary/20">
                  <CardContent className="p-4 lg:p-6">
                    <div className="flex flex-col sm:flex-row gap-4 lg:gap-6">
                      <div className="flex items-start gap-3 lg:gap-4 flex-1">
                        <Avatar className="h-14 w-14 lg:h-16 lg:w-16">
                          <AvatarImage src={tester.photo_url || "/placeholder.svg"} alt={tester.name} />
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-2">
                            <h4 className="text-base lg:text-lg font-semibold">{tester.name}</h4>
                          ...
                          <div className="flex items-center gap-2 text-xs lg:text-sm text-muted-foreground">
                            ...
                            <span>{getTesterAvailabilityText(tester)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center sm:items-start">
                        ...
                        >
                          ...
                          Schedule with {tester.name.split(" ")[0]}
                        </Button>
                      </div>
                    </div>
                  </CardContent>
