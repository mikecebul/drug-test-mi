...
type QueueNightlyPayload = Pick<Payload, 'find' | 'logger'>
...
export async function queueNightlyPendingClientUpdates(payload: QueueNightlyPayload): Promise<{
  ...
  while (hasNextPage) {
    ...
    for (const rawClient of batch.docs) {
      ...
      try {
        await queueRedwoodClientUpdate(clientId, getPendingFields(client), undefined, payload as Payload)
