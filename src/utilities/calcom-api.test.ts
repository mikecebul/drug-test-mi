...
import { createCalcomBooking, getCalcomEventType, listCalcomBookings } from './calcom-api'
...
describe('Cal.com API helpers', () => {
  ...
  test('finds the configured event type by owner, slug, and ID', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          data: [
    ...
    expect(String(fetchMock.mock.calls[0][0])).toContain('eventSlug=drug-test')
    expect(String(fetchMock.mock.calls[0][0])).toContain('username=midrugtest')
    expect(new Headers(fetchMock.mock.calls[0][1]?.headers).get('cal-api-version')).toBe('2024-06-14')
  })
